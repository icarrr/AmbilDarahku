...
-- Poster backfill throttle — cap retries on dead image sources (blob op saver)
ALTER TABLE IF EXISTS events ADD COLUMN IF NOT EXISTS poster_attempts INT NOT NULL DEFAULT 0;
ALTER TABLE IF EXISTS events ADD COLUMN IF NOT EXISTS poster_attempted_at TIMESTAMPTZ;
